...
#define MINNUM_KEY (INTERNALORDER - 1)
#define MAXNUM_KEY (2 * INTERNALORDER)
#define MINNUM_CHILD (MINNUM_KEY + 1)
#define MAXNUM_CHILD (MAXNUM_KEY + 1)
#define MINNUM_LEAF (LEAFORDER-1)
#define MAXNUM_LEAF (2*LEAFORDER)
enum NODE_TYPE {
    INTERNAL, LEAF
};        // 结点类型：内结点、叶子结点
...
public:
    ...
    KeyDataType<T1, CHAR_LENGTH> getKeyDataValue(int i) { return m_KeyData[i]; }
...
public:
    ...
    void setKeyData(int i, KeyDataType<T1, CHAR_LENGTH> keyData) { m_KeyData[i] = keyData; }
...
private:
    ...
    void recursive_insertLeaf(CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> parentNode,
    ...
    CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> getRootLeaf() {
        CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> root;
        CLeafNode_store.read(root, rootPossession);
        return root;
    }

    CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> getRootInternal() {
        CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> root;
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
int CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::getKeyIndex(string key) {
    int left = 0;
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
int CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::getKeyDataIndex(
        KeyDataType<T1, CHAR_LENGTH> keyData) {
    int left = 0;
    int right = getKeyNum() - 1;
    int mid;
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
int
CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::getKeyDataIndex(KeyDataType<T1, CHAR_LENGTH> keyData) {
    int left = 0;
    int right = getKeyNum() - 1;
    int mid;
    while (left <= right) {
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
int CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::getKeyIndex(string key) {
    int left = 0;
    int right = getKeyNum() - 1;
    int mid;
    while (left <= right) {
...
// CInternalNode
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::CInternalNode() {
    setType(INTERNAL);
    setKeyNum(0);
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>
CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::getChildInternal(int i,
                                                                               CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> *tree) {
    CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> tmp;
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>
CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::getChildLeaf(int i,
                                                                           CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> *tree) {
    CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> tmp;
    tree->LeafRead(tmp, m_Childs[i]);
    return tmp;
}

template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::clear(
        CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> *tree) {
    for (int i = 0; i <= m_KeyNum; ++i) {
        if (m_Childs[i] == 0) continue;
        if (childs_type == INTERNAL) {
            CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> tmp = getChildInternal(i, tree);
            tmp.clear(tree);
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void
CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::split(
        CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> &parentNode, int childIndex,
        CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> *tree) {
    CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> nNode;//分裂后的右节点
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void
CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::insert(int keyIndex, int childIndex,
                                                                     KeyDataType<T1, CHAR_LENGTH> keyData,
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::mergeChild(
        CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> parentNode,
        CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> childNode,
        int keyIndex,
        CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> *tree) {
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::removeKey(int keyIndex, int childIndex) {
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::borrowFrom(
        CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> siblingNode,
        CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> parentNode,
        int keyIndex,
        SIBLING_DIRECTION d,
        CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> *tree) {
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
int CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::getChildIndex(string key, int keyIndex) {
...
// CLeafNode
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::CLeafNode() {
    setType(LEAF);
    m_LeftSibling = m_RightSibling = 0;
    setKeyNum(0);
    for (int i = 0; i < MAXNUM_LEAF; ++i)
        fill(m_KeyData[i].KeyValue, m_KeyData[i].KeyValue + CHAR_LENGTH, '\0');
}

template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::~CLeafNode() = default;;

template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::clear(
        CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> *tree) {
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>
CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::getRightSibling(
        CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> *tree) const {
    CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> tmp;
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
int CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::getRightPossession() {
    return m_RightSibling;
}

template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::insert(KeyDataType<T1, CHAR_LENGTH> keyData,
                                                                      const T2 &data2) {
    int i;
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::split(
        CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> &parentNode, int childIndex,
        CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> *tree) {
    CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> nNode;//分裂后的右节点
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::mergeChild(
        CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> parentNode,
        CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> childNode, int keyIndex,
        CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> *tree) {
    // 合并数据
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::removeKey(int keyIndex,
                                                                         CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> *tree) {
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::remove(KeyDataType<T1, CHAR_LENGTH> keyData,
                                                                      CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> *tree) {
    ...
    setKeyNum(getKeyNum() - 1);
    CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> *tmp = this;
    tree->LeafUpdate(*tmp, getPossession());
}

template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::borrowFrom(
        CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> siblingNode,
        CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> parentNode,
        int keyIndex,
        SIBLING_DIRECTION d,
        CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> *tree) {
    ...
    CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> *tmp1 = this;
    tree->LeafUpdate(*tmp1, getPossession());
    tree->InteranlUpdate(parentNode, parentNode.getPossession());
}

template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::CBPlusTree(const string &FN) : CLeafNode_store(
        FN + "LeafStore", 1, 0),
                                                                                          CInternalNode_store(
                                                                                                  FN + "InternalStore",
                                                                                                  1, 0) {
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::~CBPlusTree() {
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::insert(string key, const T1 &data1, const T2 &data2) {
    // cout<<"!!!"<<key<<endl;
    KeyDataType<T1, CHAR_LENGTH> keyData(key, data1);
    if (rootPossession == 0) {
        CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> newRoot;
        rootType = LEAF;
    ...
    if (rootType == LEAF) {
        CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> root = getRootLeaf();
        root.insert(keyData, data2);
        if (root.getKeyNum() == MAXNUM_LEAF) { // 根结点已满，分裂
            CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> newNode;  //创建新的根节点
        ...
        CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> root = getRootInternal();
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::recursive_insertLeaf(
        CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> parentNode,
        KeyDataType<T1, CHAR_LENGTH> keyData,
        const T2 &data2) {
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
bool
CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::recursive_insertInternal(
        CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> &parentNode,
        KeyDataType<T1, CHAR_LENGTH> keyData,
        const T2 &data2) {
    ...
    if (parentNode.getChildtype() == INTERNAL) {
        CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> childNode = parentNode.getChildInternal(childIndex,
                                                                                                             this);
        if (recursive_insertInternal(childNode, keyData, data2)) {
            childNode.split(parentNode, childIndex, this);
            if (parentNode.getKeyNum() == MAXNUM_KEY) return true;
            else CInternalNode_store.update(parentNode, parentNode.getPossession());
        }
        return false;
    } else {
        CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> childNode = parentNode.getChildLeaf(childIndex, this);
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::clean() {
    CLeafNode_store.clean();
    CInternalNode_store.clean();
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
bool CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::Find(string key) {
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
bool
CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::recursive_searchInternal(
        CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> pNode,
        string key) {
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
bool
CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::recursive_searchLeaf(
        CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> pNode, string key) {
    int keyIndex = pNode.getKeyIndex(key);
    if (keyIndex < pNode.getKeyNum() && pNode.compareKeyvalue(keyIndex, key) == 0)
        return true;
    else return false;
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::Delete(string key, const T1 &dataValue) {
    // cout<<"@@@"<<key<<endl;
    if (rootPossession == 0) return;
    KeyDataType<T1, CHAR_LENGTH> keyData(key, dataValue);
    if (rootType == LEAF) {
        CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> root = getRootLeaf();
        if (root.getKeyNum() == 1) {
        ...
        if (root.getKeyNum() == 1) {
            if (root.getChildtype() == INTERNAL) {
                CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> pChild1, pChild2;
                ...
                CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> pChild1, pChild2;
                pChild1 = root.getChildLeaf(0, this);
                pChild2 = root.getChildLeaf(1, this);
                if (pChild1.getKeyNum() == MINNUM_LEAF && pChild2.getKeyNum() == MINNUM_LEAF) {
                    pChild1.mergeChild(root, pChild2, 0, this);
                    CInternalNode_store.Delete(rootPossession);
                    rootPossession = pChild1.getPossession();
                    rootType = LEAF;
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void
CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::recursive_removeInternal(
        CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> parentNode,
        KeyDataType<T1, CHAR_LENGTH> keyData) {
    int keyIndex = parentNode.getKeyDataIndex(keyData);
    int childIndex = parentNode.getChildIndex(keyData, keyIndex); // 孩子结点指针索引
    if (parentNode.getChildtype() == LEAF) {
        CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> pChildNode = parentNode.getChildLeaf(childIndex, this);
        if (pChildNode.getKeyNum() == MINNUM_LEAF) {
            CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> pLeft, pRight;
            ...
            // 先考虑从兄弟结点中借
            if (childIndex > 0 && pLeft.getKeyNum() > MINNUM_LEAF) {  // 左兄弟结点可借
                pChildNode.borrowFrom(pLeft, parentNode, childIndex - 1, LEFT, this);
            } else if (childIndex < parentNode.getKeyNum() && pRight.getKeyNum() > MINNUM_LEAF) {  //右兄弟结点可借
        ...
        CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> pChildNode = parentNode.getChildInternal(
                childIndex, this);
        if (pChildNode.getKeyNum() == MINNUM_KEY) {
            CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> pLeft, pRight;
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::recursive_removeLeaf(
        CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> parentNode,
        KeyDataType<T1, CHAR_LENGTH> keyData) {
    int keyIndex = parentNode.getKeyDataIndex(keyData);
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
void CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::changeKey(
        CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> pNode,
        KeyDataType<T1, CHAR_LENGTH> oldKeyData,
        KeyDataType<T1, CHAR_LENGTH> newKeyData) {
    ...
    if (keyIndex < pNode.getKeyNum() && pNode.getKeyDataValue(keyIndex) == oldKeyData) {  // 找到
        ...
        if (pNode.getChildtype() == INTERNAL) {
            CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> pChildnode = pNode.getChildInternal(keyIndex,
                                                                                                             this);
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
Vector<T2> CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::FindAll(string compareKey) {
    Vector<T2> ans;
    if (m_DataHead == 0)
        return ans;
    int start = searchFind(compareKey);
    CLeafNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> it;
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
int CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::searchFind(string key) {
...
template<class T1, class T2, int CHAR_LENGTH, int INTERNALORDER, int LEAFORDER>
int
CBPlusTree<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER>::recursive_searchFindInternal(
        CInternalNode<T1, T2, CHAR_LENGTH, INTERNALORDER, LEAFORDER> pNode,
        string key) {
    int keyIndex = pNode.getKeyIndex(key);
