...
public:
    ...
    MemoryRiver(string _FN="",bool _Ismemoryrecycling=0,bool _IsRollback=0,int _capacity=50):cacheMax(_capacity){
	...
	void read(T &tmp,int possession) {
		if(hashmap.find(possession)!=hashmap.end())tmp=hashmap[possession];
    ...
    void Delete(int possession,int type=0) {//type=1时不回收空间,保证删除的是最后一个(同时保证IsRollback=0)
		...
		if(hashmap.find(possession)!=hashmap.end())hashmap.erase(hashmap.find(possession));
    }
