...
void rollback(){
	...
	而对于MemoRiver数据库UserData，同样要记录write/Delete/update数据，但由于操作并未完全对称，所以还要大改MemoRiver的内部代码
...
void rollback(){
	...
	而对于MemoRiver数据库TrainData,OrderData，同样要记录write/Delete/update数据，但由于操作并未完全对称，所以还要大改MemoRiver的内部代码
}
```
