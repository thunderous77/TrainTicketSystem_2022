...
创建struct For_rollback_user

```
struct For_rollback_user{
	int timestamp;
...
```
MemoryRiver<Train> TrainData("TrainData"),OrderData("OrderData");
```
...
void release_train(){
    ...
    调用queueQuery()查看该train中的queue中的订单是否能补上票
}
void query_train(){
...
创建struct For_rollback_train

```
struct For_rollback_train{
	int timestamp;
    int tp;//判断存储的是哪种类型的数据
    Train train;
    Order order;
...
```
手写vector并要求能过实现sort(.begin(),.end(),cmp)
```
